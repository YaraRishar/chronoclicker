...
class CommandHandler:
    def __init__(self, driver: DriverWrapper,
        ...
        self.page: PageWrapper = self.driver.page

        self.settings_manager = settings_manager
        self.gui = gui
        self.logger = settings_manager.get_logger()
        self.settings = settings_manager.get_settings()
        self.gamedata = settings_manager.get_gamedata()
        self.aliases = settings_manager.get_aliases()
    ...
    async def comm_handler(self, comm: str) -> float | int | bool:
        ...
        if main_comm not in self.comm_dict.keys():
            ...
            partial(self.gui.root.after, 0, self.gui.update_log)()
            return False

        await self.gui.check_paused(0.1)
        if comm == main_comm:
            result = await self.driver.execute_browser_action(self.comm_dict[main_comm])
        else:
            result = await self.driver.execute_browser_action(self.comm_dict[main_comm], args)
        partial(self.gui.root.after, 0, self.gui.update_log)()
        return result
    ...
    async def loop_comm(self, comm):
        while not(self.gui.stop_event.is_set() or self.gui.script_task is None):
            await self.multi_comm_handler(comm)
            await self.gui.trigger_long_break()
    ...
    def run_script(self, comm_str=None):
        self.gui.pause_event.clear()
        self.gui.stop_event.clear()
        self.gui.comm_entry.select_clear()

        if comm_str is None:
            comm_str = self.gui.comm_entry.get()
            self.gui.previous_comms.append(comm_str)
            self.logger.info(f">>> {comm_str}")
            self.gui.comm_entry.delete(0, tk.END)
        self.gui.script_task = asyncio.run_coroutine_threadsafe(
            self.parse_command(comm_str),
            self.driver.driver_loop)

    def do_login(self):
        mail = self.gui.mail_entry.get()
        password = self.gui.password_entry.get()
        if not (mail or password):
            return
        self.gui.login_button.config(text="Загрузка...")
        Thread(target=self.run_login_sequence, args=(mail, password)).start()
        # self.run_login_sequence(mail, password)

    def run_login_sequence(self, mail, password):
        asyncio.run_coroutine_threadsafe(
            self.page.login_sequence(mail, password),
            self.driver.driver_loop)
        seconds = self.settings["max_waiting_time"]
        time.sleep(seconds)
        if "login" not in self.page.get_url():
            self.gui.show_main_screen()
            return
        self.gui.login_button.config(text="Войти")
        self.gui.login_status_label.config(text="Ошибка.\nНеверный логин \nили пароль.")

    async def logout(self):
        await self.page.goto(title=self.settings["catwar_url"])
        await self.page.wait_silent(1, 2)
        await self.page.click_element(xpath="//a/div[@title='Выход']")
        await self.page.accept_popup()

        # не знаю почему так(
        await self.page.click_element(xpath="//a/div[@title='Выход']")
        await self.page.accept_popup()

        await self.page.wait_silent(0.5, 1.5)
        await self.page.click_element(xpath="//a/div[@title='Вход']")

        await self.page.wait_silent(0.3, 1)
        self.gui.decide_on_current_screen()


    async def info(self) -> bool:
        """Команда для вывода информации о состоянии игрока из Игровой. Использование:
        info"""
        current_url = self.page.get_url()
        if current_url != f"{self.settings['catwar_url']}/cw3/":
            self.logger.info("not in cw3")
            return False

        current_location = await self.page.get_current_location()
        self.logger.info(f"Текущая локация: {current_location}\n"
                         f"Доступные локации: "
                         f"{', '.join(await self.page.get_available_locations())}\n"
                         f"Доступные действия: "
                         f"{', '.join(await self.page.get_available_actions())}")
        await self.page.print_cats()
        self.logger.info(f"\tЗдоровье:\t\t{await self.page.get_parameter('health')}%\n"
                         f"\t Бодрость:\t\t{await self.page.get_parameter('dream')}%\n"
                         f"\t Чистота:\t\t{await self.page.get_parameter('clean')}%\n"
                         f"\t Голод:\t\t{await self.page.get_parameter('hunger')}%\n"
                         f"\t Жажда:\t\t{await self.page.get_parameter('thirst')}%\n"
                         f"\t Нужда:\t\t{await self.page.get_parameter('need')}%")

        self.logger.info("Последние 5 записей в истории (введите hist, чтобы посмотреть полную историю):")
        hist_list = await self.page.get_hist_list()
        self.logger.info(f"\t{'.\n\t'.join(hist_list[-6:])}.")
        return True

    async def go(self, args=None):
        if args is None:
            self.logger.info("Для перехода нужны аргументы. Наберите help для вывода дополнительной информации.")
            return
        for index in range(len(args)):
            success = await self.page.move_to_location(args[index], self.gui)
            if not success:
                continue
    ...
    async def patrol(self, args=None):
        ...
        if len(args) == 1:
            while True:
                await self.page.move_to_location(args[0], self.gui)

        index, direction = -1, 1
        while True:
            if self.gui.stop_event.is_set() or self.gui.script_task is None:
                return False
    ...
    async def do(self, args=None, show_availables=True) -> bool:
        ...
        for action in args:
            if self.gui.stop_event.is_set() or self.gui.script_task is None:
                return False
            if await self.page.is_action_active():
                seconds = await self.page.check_time()
                await self.gui.print_timer(console_string="Действие уже совершается", seconds=seconds)
                return False
            available_actions = await self.page.get_available_actions()
            if action not in available_actions:
                ...
                continue
            action_active_sec = await self.page.check_time()
            if action_active_sec != 1:
                await self.gui.print_timer(seconds=action_active_sec + self.settings["short_break_duration"][1])
            await self.page.click_element(
                f"//a[@data-id={self.action_dict[action]}]/img",
                random_offset=(30, 30))
            seconds = await self.page.check_time() + random.uniform(
                self.settings["short_break_duration"][0],
                self.settings["short_break_duration"][1])
            await self.gui.print_timer(seconds=seconds)
            if action == "принюхаться":
                self.logger.info(await self.page.check_skill
                ("smell", clicker_utils.get_key_by_value(self.skills_dict, "smell")))
                await self.page.click_element(xpath="//tr[@id='tr_tos']/td/table/tbody/tr/td[1]/button")
            ...
            if show_availables:
                self.logger.info(f"Доступные действия: {', '.join(
                    await self.page.get_available_actions())}")
        return True

    async def smell(self):
        seconds_until_action = await self.page.check_smell_timer()
    ...
    async def wait_verbose(self, seconds=None) -> bool:
        ...
        if seconds is None or len(seconds) > 2:
            ...
            return False
        if len(seconds) == 1:
            await self.gui.print_timer(console_string="Начато ожидание",
        ...
        await self.gui.print_timer(console_string="Начато ожидание", seconds=seconds)
        return True

    async def print_inv(self):
        """ Напечатать инвентарь """
        inv_ids = await self.page.get_inv_items()
    ...
    async def pathfind_handler(self, end: tuple, forbidden_cages_given=()):
        ...
        cages = await self.page.get_cages_list()
    ...
    async def find_my_coords(self, verbose=True) -> tuple:
        my_info = await self.page.find_cat_on_loc([self.settings["my_id"]])
        my_coords = my_info[2:]
        if verbose:
            current_location = self.page.get_current_location()
    ...
    async def check_parameter(self, args=None) -> float | int:
        ...
        param_value = await self.page.get_parameter(param_name=param_name_server)
    ...
    async def check_skill(self, args=None) -> int:
        ...
        skill_value = await self.page.check_skill(skill_name_server, skill_name)
    ...
    async def print_rabbits_balance(self):
        ...
        await self.page.goto(f"{self.settings['catwar_url']}/rabbit")
        rabbit_balance = clicker_utils.get_text(await self.page.locate_element(
    ...
    async def start_rabbit_game(self) -> bool:
        ...
        await self.page.goto(f"{self.settings['catwar_url']}/chat")
        await self.page.wait_silent(1, 3)
        await self.page.click_element(xpath="//a[@data-bind='openPrivateWith_form']")
        await self.page.type_in_chat("Системолап", entry_xpath="//input[@id='openPrivateWith']")
        await self.page.click_element(xpath="//*[@id='openPrivateWith_form']/p/input[2]")

        games_played = 0
        while games_played != 5:
            await self.page.rabbit_game()
            games_played += 1
        await self.page.goto(f"{self.settings['catwar_url']}/cw3/")
        return True
    ...
    async def do_action_with_cat_handler(self, args=None):
        ...
        await self.page.do_action_with_cat(cat_name)
    ...
    #         return
    #     inv_items = await self.page.get_inv_items()
    #     if not inv_items:
    ...
    #     await self.page.click_element(xpath=f"//div[@class='itemInMouth']/img[@src='things/{item_img_id}.png']",
    #                             random_offset=(10, 10))
    #     await self.page.wait_silent(0.3, 0.6)
    #     slider_xpath = "//div[@id='layer']/span[@class='ui-slider-handle ui-state-default ui-corner-all']"
    #     await self.page.click_element(slider_xpath)
    #     while level != 1:
    #         await self.page.wait_silent(0.1, 0.5)
    #         await self.page.type_in_chat(entry_xpath=slider_xpath, text="")
    #
    #         slider.send_keys(Keys.ARROW_RIGHT)
    #         level -= 1
    #     await self.page.click_element(xpath="//a[text()='Закопать']")
    #     seconds = await self.page.check_time() + random.uniform(
    ...
    async def cancel(self) -> bool:
        ...
        success = await self.page.click_element(xpath="//a[@id='cancel']")
        if success:
            self.gui.stop_event.set()
            self.logger.info("Действие отменено!")
            self.gui.timer.set("Действие не выполняется.")
            return True
    ...
    async def clear_hist(self):
        ...
        await self.page.click_element(xpath="//a[@id='history_clean']")
...
class ClickerGui:
    def __init__(self, command_handler: CommandHandler):
        ...
        self.settings = command_handler.settings
        self.logger = command_handler.logger
        self.logfile_path = command_handler.settings_manager.get_logpath()

        self.pause_event = asyncio.Event()
        self.stop_event = asyncio.Event()

        self.script_task = None
        self.last_log_idx: int = 0
        self.previous_comms: list = []
        self.last_comm_idx: int = -1
        self.password_show_toggle: bool = True
        ...
        self.loading_var.set("Загрузка... \nЕсли это сообщение не исчезает, "
                             "\nпопробуйте запустить кликер от админа \nили проверить соединение с интернетом.")
        ...
        self.login_button = tk.Button(self.login_frame, text="Войти", command=self.command_handler.do_login)
        ...
        partial(self.root.after, 0, self.initialize_driver)()
    ...
    def initialize_driver(self):
        # cringe starts
        while not self.command_handler.driver.page:
            time.sleep(0.1)
        while "catwar" not in self.command_handler.driver.page.get_url():
            time.sleep(0.1)
        # cringe stops... unless......

        self.decide_on_current_screen()
        self.command_handler.page = self.command_handler.driver.page
        # while "cw3" or "login" not in self.command_handler.driver.page.get_url():
        #     time.sleep(0.1)


    def decide_on_current_screen(self):
        if "login" in self.command_handler.driver.page.get_url():
    ...
    def show_main_screen(self):
        ...
        catwar_url = self.settings["catwar_url"] + "/cw3/"

        if "cw3" not in self.command_handler.driver.page.get_url():
            asyncio.run_coroutine_threadsafe(self.command_handler.driver.page.goto(catwar_url),
                                             self.command_handler.driver.driver_loop)
        while "cw3" not in self.command_handler.driver.page.get_url():
            time.sleep(0.1)
        self.command_handler.run_script("info")
    ...
    #     # asyncio.run_coroutine_threadsafe(self.run_script(), self.command_handler.driver.driver_loop)
    #     asyncio.run_coroutine_threadsafe(self.test(), self.command_handler.driver.driver_loop)
    #     partial(self.root.after, 0, self.update_log)()
    ...
    def ok_button_pressed(self, _event=None):
        self.command_handler.run_script()
        partial(self.root.after, 0, self.update_log)()
        if not self.ensure_status():
            self.timer.set("Что-то пошло не так. Проверьте соединение с интернетом!")
        self.last_comm_idx = -1
    ...
    async def print_timer(self, seconds: float, console_string=None):
        ...
        if console_string is not None:
            for i in range(round(seconds), -1, -1):
                ...
                await self.command_handler.page.wait_silent(1, do_random=False)
            self.timer.set("Действие не выполняется.")
            return

        message = await self.command_handler.page.get_action_str()
    ...
    def ensure_status(self):
        ...
        if self.command_handler.driver and "catwar" in self.command_handler.driver.page.get_url():
            return True
        return False
    ...
    def on_close(self):
        self.stop_event.set()
        self.pause_event.clear()
        if self.command_handler.driver is not None:
            self.command_handler.driver.close_browser()
            partial(self.command_handler.driver.driver_loop.call_soon_threadsafe,
                    self.command_handler.driver.driver_loop.stop)()
...
if __name__ == "__main__":
    ...
    browser = DriverWrapper(setting_manager)
    comm_handler = CommandHandler(browser, setting_manager)
    clicker_gui = ClickerGui(comm_handler)
    comm_handler.gui = clicker_gui

    try:
        clicker_gui.mainloop()
    finally:
